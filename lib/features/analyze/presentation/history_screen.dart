...
class _HistoryScreenState extends State<HistoryScreen> {
  ...
  Future<void> _deleteRun(String id) async {
    ...
    try {
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('Analysis item deleted')));
    } catch (e) {
      if (!mounted) return;
  ...
  Future<void> _ensureSegments(String reportId) async {
    ...
    final segments = raw
        ...
        .map((e) => AnalysisSegment.fromJson(Map<String, dynamic>.from(e)))
        .toList();
          ...
          onTap: () async {
            ...
            if (controller != null && controller.value.isInitialized) {
              await controller.seekTo(
                Duration(seconds: segment.startSec.round()),
              );
              controller.play();
            }
          },
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          if (segment.recommendation != null &&
              segment.recommendation!.isNotEmpty) ...[
            const Divider(color: Colors.white10, height: 1),
            // AI Recommendation
            Padding(
              padding: const EdgeInsets.all(20),
        ...
        builder: (context, analysisService, child) {
          ...
          return ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              final highSeverityCount = segmentList
                  .where(
                    (s) =>
                        s.severityLabel.toUpperCase() == 'HIGH' ||
                        s.severityLabel.toUpperCase() == 'CRITICAL',
                  )
                  .length;
                ...
                child: Card(
                  ...
                  child: Column(
                    children: [
                      ListTile(
                        title: Text(
                          report.inputVideoName ??
                              'Analysis ${report.id.substring(0, 6)}',
                        ),
                        subtitle: Text(
                            ...
                            IconButton(
                              ...
                              onPressed: () async {
                                ...
                                if (!wasExpanded) {
                                  ...
                                  final videoUrl =
                                      report.outputs?['output_video'] ??
                                      report.outputs?['original_video'];
                                  if (videoUrl != null &&
                                      _controllers[report.id] == null) {
                                    final controller =
                                        VideoPlayerController.networkUrl(
                                          Uri.parse(videoUrl),
                                        );
                                    _controllers[report.id] = controller;
                                    _initFutures[report.id] = controller
                                        .initialize()
                                        .then((_) => setState(() {}));
                                  }
                                } else {
                        ...
                        onTap: () {
                          Navigator.of(context).push(
                            MaterialPageRoute(
                              builder: (_) =>
                                  AnalysisPreviewScreen(report: report),
                            ),
                          );
                        },
                      ),
                      Padding(
                        ...
                        child: Row(
                          children: [
                            Text(
                              'Segments: ${report.outputs?['segments_count'] ?? segmentCount}',
                            ),
                              ...
                              children: [
                                ...
                                if (segmentList.isNotEmpty)
                                  _buildFocusedTelemetry(
                                    report.id,
                                    segmentList[_selectedSegmentIndices[report
                                            .id] ??
                                        0],
                                  ),
                              ],
                            ...
                            child: Column(
                              children: segmentList.map((segment) {
                                return SegmentCard(
                                  ...
                                  onPlay: () async {
                                    setState(() {
                                      _selectedSegmentIndices[report.id] =
                                    ...
                                    if (controller != null &&
                                      ...
                                      await controller.seekTo(
                                        Duration(
                                          seconds: segment.startSec.round(),
                                        ),
                                      );
                                      controller.play();
                                    }
